...
- `breakdown`: an itemised list of lines in fixed order: `base`, `rush`, `weight`, `small-order`, `delivery-window`, and when needed a final `cap`
...
| Total fee over `2000` cents after all charges   | final `cap` line reducing the total to `2000` cents |

Free delivery waives only the `base` line. Rush, weight, small-order, and delivery-window surcharges still apply. A maximum delivery fee of `2000` cents is then enforced with a final negative `cap` line when needed, and the total always equals the sum of the returned `breakdown` lines.
